...
\begin{center}
  ...
  \Huge
  \textbf{Trabajo Práctico: Estadística Acturial}
...
head(tabla_cunatia, 10)%>%
  kbl(caption = "Cuantías Pagadas en 2023.", align = "c") %>% # Añadir un título
...
tabla1$`Margen de Solvencia Mínimo Burr` <-
  paste0("$", formatC(tabla1$`Margen de Solvencia Mínimo Burr`,
                       big.mark = ".", format = "f", digits = 0))
tabla1 %>%
  kbl(caption = "Margen de Solvencia Mínimo según porcentaje de recargo de seguridad.", align = "c") %>% # Añadir un título
  kable_styling(bootstrap_options = "striped", full_width = FALSE) %>% 
  row_spec(0, background = "#66CDAA")
```
